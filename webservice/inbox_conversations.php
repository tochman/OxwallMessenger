...
SELECT interlocutorId AS receiverID, 
initiatorId, 
subject, 
viewed, 
textValue AS realname, 
ow_mailbox_conversation.id AS conversation_id, 
ow_base_question_data.userId AS userID, 
ow_base_avatar.hash,
`read` AS conversationflag,
COUNT(ow_mailbox_message.id) AS messagecount

FROM ow_mailbox_conversation
INNER JOIN 

ow_mailbox_message ON
ow_mailbox_conversation.id = ow_mailbox_message.conversationId
...
GROUP BY ow_mailbox_conversation.id
...
if ($rows) {
    ...
    foreach ($rows as $row) {
        ...
        $post["messagecount"]    = intval($row["messagecount"]);
        $post["conversationflag"]    = intval($row["conversationflag"]);
        $post["startedbyid"] = $row["initiatorId"];
